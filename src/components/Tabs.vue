<template>
    <div class="Tabs">
        ...
        <div v-for="card,index in tabs" :key="index" class="Tabs-Header-Items px-6 py-4 mx-3 cursor-pointer" :class=" index === activeTab? 'text-customBlue border-b-4 border-customBlue' :  'border-b'" @click="updateActiveTab(index)">
            {{card.title}}
        </div>
        </header>
        <div class="container mx-auto bg-contain bg-no-repeat flex items-center justify-center" :style="`background-image:url(${getActiveDataTab.imgSrc}); min-height:600px`">
           <div class="mb-16 text-white">
             <h2 class="text-4xl uppercase text-center font-bold mb-4">{{getActiveDataTab.title}}</h2>
             <div class="bg-white h-2 w-16 mx-auto mb-16" />
             <p class="text-center mx-8 lg:mx-24 text-center">{{getActiveDataTab.description}}</p>
           </div>
        </div>
    </div>
</template>
